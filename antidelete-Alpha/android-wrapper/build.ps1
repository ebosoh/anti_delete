...
$verifyArgs = @(
    ...
    $APKSIGNER,
    "verify",
    "antidelete-Alpha.apk"
)
Start-Process cmd.exe -ArgumentList $verifyArgs -NoNewWindow -Wait

# Check result size
if (Test-Path antidelete-Alpha.apk) {
    $size = (Get-Item antidelete-Alpha.apk).Length
    $sizeKb = [Math]::Round($size / 1024, 2)
    Write-Host "APK Compiled Successfully! Output file: antidelete-Alpha.apk" -ForegroundColor Green
    Write-Host "Final APK size: $sizeKb KB" -ForegroundColor Green
    Copy-Item antidelete-Alpha.apk ..\antidelete-Alpha.apk -Force
    Write-Host "Copied APK to web server directory: antidelete-Alpha/antidelete-Alpha.apk" -ForegroundColor Green
} else {
